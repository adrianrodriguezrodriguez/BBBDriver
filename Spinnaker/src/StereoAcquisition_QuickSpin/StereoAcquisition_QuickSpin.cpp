...
/**
 *  @example StereoAcquisition_QuickSpin.cpp
 *
 *  @brief StereoAcquisition_QuickSpin.cpp shows how to acquire image sets from a stereo
 *  camera using the QuickSpin API. QuickSpin is a subset of the Spinnaker library
 *  that allows for simpler node access and control.
 *  The image sets are then saved to file and/or used to compute 3D
