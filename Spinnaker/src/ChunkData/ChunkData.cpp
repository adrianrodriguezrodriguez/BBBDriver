...
void printBytesAsHex(uint8_t* array, int64_t length)
{
    ...
    for (int64_t i = 0; i < length - 1; ++i)
    {
        ...
        cout << "0x" << hex << setw(2) << setfill('0') << static_cast<int>(array[i]) << " ";
    }
    cout << endl;
}
...
int DisplayChunkData(ImagePtr pImage)
{
    ...
    try
    {
        ...
        int64_t serialDataLength = chunkData.GetSerialDataLength();
        if (serialDataLength > 0)
        {
            cout << "\tSerial Data (" << serialDataLength << ") Bytes):" << endl;
            if (chunkData.GetSerialReceiveOverflow())
            {
                cout << "\t    Warning: Device serial data buffer overflow" << endl;
            }
            uint8_t* pSerialData = chunkData.GetSerialData();
            printBytesAsHex(pSerialData, chunkData.GetSerialDataLength());
        }
        else
        {
            cout << "\tSerialData: N/A" << endl;
        }
    }
    catch (Spinnaker::Exception& e)
    {
...
int DisplayChunkData(INodeMap& nodeMap)
{
    ...
    try
    {
        ...
        for (it = features.begin(); it != features.end(); ++it)
        {
            ...
            if (pFeature->GetPrincipalInterfaceType() == intfIBoolean)
            {
